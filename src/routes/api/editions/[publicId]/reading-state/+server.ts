...
export const PATCH: RequestHandler = handleApiErrorWrapper(async (event) => {
    ...
    const data = await getFormDataOrJson<InferIn<ReadingStateUpdateSchema>>(event)
    if (!data) {
        throw new ApiError(HttpCodes.ClientError.BadRequest, "No Data Provided")
    }
    const form = await schemaValidator(data, ReadingStateUpdateSchema)
    const result = await api.editions.publicId.readingState.PATCH(event.params.publicId, form.data, event.locals)
