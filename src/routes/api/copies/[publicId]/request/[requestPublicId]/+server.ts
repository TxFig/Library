...
import { CopyRequestUpdateSchema } from "$lib/validation/interactions/copy";
...
export const PATCH: RequestHandler = handleApiErrorWrapper(async (event) => {
    ...
    const data = await getFormDataOrJson<v.InferInput<CopyRequestUpdateSchema>>(event)
    if (!data) {
        throw new ApiError(HttpCodes.ClientError.BadRequest, "No Data Provided")
    }

    const form = await schemaValidator(data, CopyRequestUpdateSchema)

    const result = await api.copies.request.PATCH(
