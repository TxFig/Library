...
<div class="flex flex-col gap-4 p-6">
    ...
    {:else}
        ...
        <form class="flex flex-col gap-4" method="post" use:enhance inert={successfulCreation}>
            <div>
                <TextInputField text="Email" bind:value={$form.email} errors={$errors.email} placeholder="Enter email..." required name="email" />
                <ErrorMessage errors={$errors.email} />
            </div>
            <div>
                <TextInputField text="Username" bind:value={$form.username} errors={$errors.username} placeholder="Enter username..." required name="username" />
